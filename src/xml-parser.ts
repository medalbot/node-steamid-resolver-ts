 ...
 * Uses Bun's built-in fetch and DOMParser for optimal performance
 */

import { parseString } from 'xml2js';
import type { RawXMLParseResult, SteamXMLResponse } from './types.js';
...
import { debugLog, extractString, withRetry } from './utils.js';
...
async function fetchSteamXML(url: string): Promise<string> {
  ...
  try {
    const response = await fetch(url, {
      headers: {
        'User-Agent': 'Mozilla/5.0 (compatible; steamid-resolver-ts/2.0)',
      },
    });
